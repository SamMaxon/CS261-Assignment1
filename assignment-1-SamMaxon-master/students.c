...
struct dynarray* create_student_array(int num_students, char** names, int* ids,
	...
	struct dynarray*  s_arr = dynarray_create();	
	for(i = 0; i < num_students; i++){
		struct student* temp = create_student(names[i], ids[i], gpas[i]);
		dynarray_insert(s_arr, i, temp);
	}
...
void print_students(struct dynarray* students) {
	...
	for(i = 0; i < dynarray_size(students); i++){
		struct student* temp = dynarray_get(students, i);
...
struct student* find_max_gpa(struct dynarray* students) {
	...
	return dynarray_get(students, max);
}
...
struct student* find_min_gpa(struct dynarray* students) {
	...
	return dynarray_get(students, min);
}
...
void sort_by_gpa(struct dynarray* students) {
	...
	for(i = 0; i < dynarray_size(students) - 1; i++){
		...
		for(j = 0; j < dynarray_size(students) - i - 1; j++){
			struct student* temp1 = dynarray_get(students, j);
			struct student* temp2 = dynarray_get(students, j+1);
			if(temp1->gpa < temp2->gpa){
				struct student* temp3 = temp1;
				dynarray_set(students, j, temp2);
				dynarray_set(students, j+1, temp3);
			}
